document.addEventListener("DOMContentLoaded", () => {
  ...
  if (form) {
    form.addEventListener("submit", async (e) => {
      ...
      const telegram_id = parseInt(telegramIdInput.value.trim(), 10);
      ...
      try {
        const response = await fetch(
          "https://kakaskibackend.onrender.com/create_user",
          {
            method: "POST",
            headers: {
        ...
        alert(`Користувача створено! ID: ${data.id}`);
      } catch (err) {
  ...
  async function getUserByTelegramId(telegramId) {
    return await sendApiRequest(
      `${API_URL}/users/by_telegram_id/?telegram_id=${telegramId}`
    );
  }

  async function updateUserData(userIdToUpdate, dataToUpdate) {
    await sendApiRequest(
      `${API_URL}/users/${userIdToUpdate}`,
      "PATCH",
      dataToUpdate
    );
  }
  ...
  function spawnPoopParticles(event) {
    ...
    for (let i = 0; i < clicksPerTap; i++) {
      // Кількість спавнених какашок = clicksPerTap
  ...
  async function renderLeaderboard() {
    leaderboardList.innerHTML =
      '<li class="loading-message">Завантаження лідерів...</li>';
    ...
    try {
      ...
      if (leaders.length === 0) {
        leaderboardList.innerHTML =
          '<li class="empty-message">Ще немає лідерів :( Будь першим!</li>';
        return;
      }
  ...
  function renderShopItems() {
    ...
    shopProducts.forEach((product) => {
      ...
      itemDiv.innerHTML = `
            ...
            }>${product.cost.toLocaleString()} 💩</button>
        `;
  ...
  appContainer.addEventListener("click", (event) => {
    ...
    if (event.target.matches(".shop-item .buy-btn")) {
      const itemId = parseInt(
        event.target.closest(".shop-item").dataset.itemId
      );
      buyItem(itemId);
    }
  });
  ...
  async function init() {
    ...
    try {
      ...
      tg.showAlert(
        `Ласкаво просимо, ${userName}! Ваш баланс: ${score.toLocaleString()}`
      );
    } catch (error) {
      console.warn(
      ...
      try {
        ...
        tg.showAlert(
          `Вітаємо, ${userName}! Ви новий гравець. Починайте клікати!`
        );
      } catch (createError) {
        console.error(
          "Критична помилка при ініціалізації користувача:",
          createError
        );
        tg.showAlert(
