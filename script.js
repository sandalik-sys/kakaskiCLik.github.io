...
let userId = null;
let userName = 'Гість';
let clicksPerTap = 1; // Нова змінна для апгрейдів
...
const poopSpawnArea = document.getElementById('poop-spawn-area'); // Новий елемент для спавну какашок
...
// --- API Endpoint (переконайся, що цей URL доступний і твій бекенд працює!) ---
const API_URL = 'https://bc4f424f-769c-4cbf-8349-06cdfb818e25-00-pb50i88fjg02.worf.replit.dev';

// --- Дані магазину (можна завантажувати з API або мати статично) ---
const shopProducts = [
    ...
    // Додай більше товарів за бажанням
];
...
async function createUser(name, telegramId, initialClicks = 0, initialClicksPerTap = 1) {
    try {
        return await sendApiRequest(`${API_URL}/users/`, 'POST', {
            ...
            clicks_per_tap: initialClicksPerTap // Додаємо clicks_per_tap при створенні
        });
    } catch (error) {
